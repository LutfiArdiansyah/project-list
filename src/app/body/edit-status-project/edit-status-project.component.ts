...
@Component({
  selector: 'app-edit-status-project',
  templateUrl: './edit-status-project.component.html',
  styleUrls: ['./edit-status-project.component.css']
})
export class EditStatusProjectComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.id_status_project) {
      this.loading = true;
      this.services.postData('get_mst_status_project',{id:this.id_status_project}).subscribe(response => {
  ...
  edit() {
    ...
    this.services.postData('update_status_project',this.status_projects).subscribe(response => {
