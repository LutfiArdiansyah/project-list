...
@Component({
  selector: 'app-edit-tipe-project',
  templateUrl: './edit-tipe-project.component.html',
  styleUrls: ['./edit-tipe-project.component.css']
})
export class EditTipeProjectComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.id_status_project) {
      this.loading = true;
      this.services.postData('get_mst_tipe_project',{id:this.id_status_project}).subscribe(response => {
  ...
  edit() {
    ...
    this.services.postData('update_tipe_project',this.status_projects).subscribe(response => {
